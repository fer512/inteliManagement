...
import ar.com.intelimanagement.service.CountryService;
...
import ar.com.intelimanagement.service.CountryQueryService;
...
public class CountryResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllCountriesByProvincesIsEqualToSomething() throws Exception {
        // Initialize the database
        Province provinces = ProvinceResourceIntTest.createEntity(em);
        em.persist(provinces);
        em.flush();
        country.addProvinces(provinces);
        countryRepository.saveAndFlush(country);
        Long provincesId = provinces.getId();

        // Get all the countryList where provinces equals to provincesId
        defaultCountryShouldBeFound("provincesId.equals=" + provincesId);

        // Get all the countryList where provinces equals to provincesId + 1
        defaultCountryShouldNotBeFound("provincesId.equals=" + (provincesId + 1));
    }
